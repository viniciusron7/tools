...
function selectDoor(doorIndex) {
  ...
  const goatDoors = [0, 1, 2].filter(
    (i) => i !== currentGame.selectedDoor && i !== currentGame.carDoor,
  );
  currentGame.openedDoor =
    goatDoors[Math.floor(Math.random() * goatDoors.length)];

  setTimeout(() => {
    doors[currentGame.openedDoor].querySelector(".door").classList.add("open");
    doors[currentGame.openedDoor]
      .querySelector(".door-content")
      .classList.add("show");
...
function makeDecision(switched) {
  ...
  if (switched) {
    const otherDoor = [0, 1, 2].find(
      (i) => i !== currentGame.selectedDoor && i !== currentGame.openedDoor,
    );
...
function runNextSimulation() {
  ...
  const goatDoors = [0, 1, 2].filter(
    (i) => i !== simGame.selectedDoor && i !== simGame.carDoor,
  );
  ...
  if (simGame.switched) {
    const otherDoor = [0, 1, 2].find(
      (i) => i !== simGame.selectedDoor && i !== simGame.openedDoor,
    );
...
function updateStats() {
  ...
  document.getElementById("switchWinRate").textContent =
    switchedGames.length > 0
      ? Math.round((switchedWins / switchedGames.length) * 100) + "%"
      : "0%";
  document.getElementById("stayWinRate").textContent =
    stayedGames.length > 0
      ? Math.round((stayedWins / stayedGames.length) * 100) + "%"
      : "0%";
...
doors.forEach((doorContainer, index) => {
  doorContainer.addEventListener("click", () => {
    if (
      gamePhase === "selecting" &&
      !doorContainer.classList.contains("disabled")
    ) {
      selectDoor(index);
    }
  });
