...
async function copyImagePNG() {
  const latex = getLatex();
  if (!latex.trim()) {
    showToast("Nenhuma equação para copiar");
    return;
  }
...
function svgToCanvas(svg, scale) {
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onerror = () =>
      reject(new Error("Falha ao renderizar SVG como imagem"));
    img.src = dataUrl;
  });
}
...
async function exportSVG() {
  const latex = getLatex();
  if (!latex.trim()) {
    showToast("Nenhuma equação para exportar");
    return;
  }
...
function buildPalette() {
  ...
  for (const cat of SYMBOL_CATEGORIES) {
    ...
    btn.className =
      "palette-tab" + (cat.id === activeCategory ? " active" : "");
...
function initResizableHandles() {
  ...
  function applySizes() {
    panelsEl.style.gridTemplateColumns = `${sizes[0]}fr 6px ${sizes[1]}fr`;
  }
