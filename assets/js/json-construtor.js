...
function toggleSelectionMode() {
  ...
  if (selectionMode) {
    ...
    updateSelectedCount();
    showToast(
      "Modo seleção múltipla ativado. Clique nos elementos para selecioná-los.",
      "success",
    );
  } else {
...
function selectElement(elementId, path, key) {
  ...
  if (index > -1) {
    ...
    childElements.forEach((child) => {
      ...
      const childIdx = selectedElements.findIndex(
        (sel) => sel.elementId === childId,
      );
      if (childIdx > -1) {
    ...
    childElements.forEach((child) => {
      ...
      if (
        childId &&
        !selectedElements.find((sel) => sel.elementId === childId)
      ) {
        try {
...
function removeSelectedElements() {
  ...
  if (
    confirm(
      `Tem certeza que deseja remover ${selectedElements.length} elemento(s) selecionado(s)?`,
    )
  ) {
    saveState();
    ...
    try {
      ...
      updatePreview();
      showToast(
        `${sortedElements.length} elemento(s) removido(s) com sucesso.`,
        "success",
      );
    } catch (error) {
...
function filterTopLevelSelected() {
  return selectedElements.filter((sel) => {
    ...
    return !selectedElements.some((other) => {
      ...
      return otherFullPath.every(
        (part, i) => String(part) === String(selFullPath[i]),
      );
    });
  });
}
...
function confirmMultiMove() {
  ...
  try {
    ...
    showToast(
      `${elementsToMove.length} elemento(s) movido(s) com sucesso!`,
      "success",
    );
  } catch (error) {
  ...
  Object.keys(obj).forEach((key) => {
    ...
    if (
      ...
      JSON.stringify(currentPath) ===
        JSON.stringify([...moveData.path, moveData.key])
    ) {
      return;
    }
    ...
    if (multiMoveData && multiMoveData.length > 0) {
      const isBeingMoved = multiMoveData.some(
        (data) =>
          JSON.stringify(currentPath) ===
          JSON.stringify([...data.path, data.key]),
      );
    ...
    if (moveData) {
      ...
      const isChildOfMoved =
        currentPath.length > movePath.length
          ? false
          : movePath
              .slice(0, currentPath.length)
              .every((p, i) => String(p) === String(currentPath[i]));
    ...
    if (Array.isArray(value)) {
      ...
      value.forEach((item, index) => {
        if (typeof item === "object" && item !== null && !Array.isArray(item)) {
          ...
          itemOption.onclick = () =>
            selectDestination(itemPath, "object", itemOption);
          ...
          itemOption.onclick = () =>
            selectDestination(itemPath, "array", itemOption);
          container.appendChild(itemOption);
...
function addElementToDestination(destination, originalKey, element) {
  try {
    ...
    console.error("Erro ao adicionar elemento ao destino:", error, {
      destination,
      originalKey,
      element,
    });
...
function importJSON() {
  ...
  try {
    ...
    if (
      Object.keys(jsonStructure).length > 0 &&
      !Array.isArray(jsonStructure) &&
      !Array.isArray(parsedJSON)
    ) {
      if (
        confirm(
          'Deseja substituir o JSON atual? Clique em "Cancelar" para mesclar com o existente.',
        )
      ) {
        saveState();
...
function startEditing(button) {
  ...
  if (isComplexValue) {
    ...
    if (isArrayItem) {
      showToast(
        "Não é possível editar diretamente arrays ou objetos dentro de arrays. Use os botões específicos para adicionar/remover elementos.",
        "error",
      );
      return;
    }
...
function startEditingComplexValue(elementNode, path, keyOrIndex, currentValue) {
  ...
  const valueDisplay = Array.isArray(currentValue)
    ? `Array [${currentValue.length} items]`
    : "Objeto";
...
function startEditingPrimitiveValue(
  elementNode,
  path,
  keyOrIndex,
  currentValue,
  ...
  if (valueType === "null") {
    valueInputHtml =
      '<span id="edit-value-container" style="color: #666;">null</span>';
  } else if (valueType === "boolean") {
    valueInputHtml = `
...
function handleDrop(e) {
  ...
  if (
    JSON.stringify(dragSourcePath) === JSON.stringify(targetPath) &&
    String(dragSourceKey) === String(targetKey)
  ) {
    dragSourcePath = null;
    dragSourceKey = null;
    return;
  ...
  if (targetFull.length >= sourceFull.length) {
    const isDescendant = sourceFull.every(
      (part, i) => String(part) === String(targetFull[i]),
    );
    if (isDescendant) {
      dragSourcePath = null;
      dragSourceKey = null;
  ...
  try {
    ...
    if (targetIsContainer) {
      ...
      const elementToMove = JSON.parse(
        JSON.stringify(getElementByPath(dragSourcePath, dragSourceKey)),
      );
      ...
      if (Array.isArray(targetValue)) {
        addElementToDestination(
          { path: [...targetPath, targetKey], type: "array" },
          originalKey,
          elementToMove,
        );
      } else {
        addElementToDestination(
          { path: [...targetPath, targetKey], type: "object" },
          originalKey,
          elementToMove,
        );
      }
      ...
      addElementToDestination(
        {
          path: targetPath,
          type: Array.isArray(targetParent) ? "array" : "object",
        },
        originalKey,
        elementToMove,
      );
...
function toggleCollapse(btn) {
  ...
  if (parentElement) {
    const nestedContent = parentElement.querySelector(
