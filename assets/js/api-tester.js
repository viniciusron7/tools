...
function switchTab(tabName, el) {
  document
    .querySelectorAll(".request-panel > .tabs .tab")
    .forEach((tab) => tab.classList.remove("active"));
  document
    .querySelectorAll(".request-panel > .tab-content")
...
function switchResponseTab(tabName, el) {
  document
    .querySelectorAll("#responseTabs .tab")
    .forEach((tab) => tab.classList.remove("active"));
  document
    .querySelectorAll('[id^="response-"][id$="-content"]')
    .forEach((content) => content.classList.remove("active"));
...
function exportHeaders() {
  ...
  const blob = new Blob([JSON.stringify(headers, null, 2)], {
...
function loadHistory() {
  ...
  if (requestHistory.length === 0) {
    historyList.innerHTML =
      '<p style="text-align: center; color: #94a3b8;">Nenhuma requisição no histórico ainda</p>';
    return;
  }

  historyList.innerHTML = requestHistory
    .map(
      (item) => `
...
function exportToCurl() {
  ...
  if (
    ...
    typeof requestData.options.body === "string"
  ) {
