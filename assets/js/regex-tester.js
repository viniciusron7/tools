...
function truncateDisplay(str, max) {
  ...
  return (
    escapeHtml(str.slice(0, max)) +
    '<em class="group-val" style="color:var(--text-muted)">…</em>'
  );
}
...
function renderHighlight(text, matches) {
  if (!text) {
    highlightOutputEl.innerHTML =
      '<span class="placeholder-text">Os destaques aparecerão aqui...</span>';
    return;
  }
...
function updateStats(matches) {
  ...
  matchBadgeEl.textContent =
    count === 1 ? "1 correspondência" : count + " correspondências";
  matchBadgeEl.classList.toggle("zero", count === 0);

  if (count === 0) {
    groupsBodyEl.innerHTML =
      '<span class="no-groups-msg">Nenhuma correspondência encontrada</span>';
    return;
  }
  ...
  if (!hasGroups) {
    groupsBodyEl.innerHTML =
      '<span class="no-groups-msg">A regex encontrou correspondências, mas não há grupos capturados. Use parênteses <code>()</code> para capturar partes do match.</span>';
    return;
  }
  ...
  displayMatches.forEach((match, i) => {
    ...
    html +=
      ...
      truncateDisplay(match[0] || "(vazio)", 60) +
      "</span>";
    ...
    for (let g = 1; g < match.length; g++) {
      ...
      if (val === undefined) {
        ...
        html +=
          '<span class="group-val">' + truncateDisplay(val, 80) + "</span>";
      }
      html += "</div>";
    }
  ...
  if (matches.length > MAX_DISPLAY_MATCHES) {
    html +=
      '<p class="more-matches-msg">… e mais ' +
