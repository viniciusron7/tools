...
function copyLatexCode() {
    ...
    navigator.clipboard.writeText(currentLatexCode).then(() => {
...
function updateCheckerboard() {
    ...
    if (format === 'JPG') {
        ...
        previewArea.style.backgroundColor = isTransparent ? '#ffffff' : bgColor.value;
...
async function convert() {
    ...
    if (!window.MathJax || !window.mathjaxReady) {
        showError('MathJax ainda está carregando. Tente novamente em alguns segundos.');
        return;
    }
    ...
    try {
        ...
        let latex;
        if (isLatexInput(rawInput)) {
            latex = rawInput;
        } else {
            try {
                latex = expressionToLatex(rawInput);
            } catch (e) {
                latex = rawInput;
            }
        }
...
function rasterize(mimeType) {
    return new Promise((resolve, reject) => {
        ...
        if (isTransparent) {
            ...
            const bg = (mimeType === 'image/jpeg' && transparentBg.checked) ? '#ffffff' : bgColor.value;
            ctx.fillStyle = mimeType === 'image/jpeg' ? (transparentBg.checked ? '#ffffff' : bgColor.value) : bgColor.value;
